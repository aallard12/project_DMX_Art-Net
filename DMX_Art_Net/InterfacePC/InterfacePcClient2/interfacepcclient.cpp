...
void InterfacePcClient::setupUi()
{
    ...
    QPushButton* btnGoToList = new QPushButton("Univers & Équipements");
    btnGoToList->setObjectName("btnTeal");
    connect(btnGoToList, &QPushButton::clicked, this, &InterfacePcClient::showList);
    headerLayout->addWidget(btnGoToList);

    QPushButton* btnAddMain = new QPushButton("+ Ajouter Équipement");
    btnAddMain->setObjectName("btnSeaGreen");
    connect(btnAddMain, &QPushButton::clicked, this, &InterfacePcClient::showAddForm);
    headerLayout->addWidget(btnAddMain);

    QPushButton* btnGoToScenes = new QPushButton("Créer les Scènes");
    btnGoToScenes->setObjectName("btnDarkCyan");
    connect(btnGoToScenes, &QPushButton::clicked, this, &InterfacePcClient::showScenesPage);
    headerLayout->addWidget(btnGoToScenes);

    QPushButton* btnGoToLive = new QPushButton("Live / Régie");
    btnGoToLive->setObjectName("btnDeepTeal");
    connect(btnGoToLive, &QPushButton::clicked, this, &InterfacePcClient::showLivePage);
    ...
    connect(uiUniversList, &QListWidget::itemSelectionChanged, this, &InterfacePcClient::onUniversSelectionChanged);
    ...
    connect(btnEditUnivers,   &QPushButton::clicked, this, &InterfacePcClient::editUnivers);
    connect(btnDeleteUnivers, &QPushButton::clicked, this, &InterfacePcClient::deleteUnivers);
    ...
    connect(btnAddUnivers, &QPushButton::clicked, this, &InterfacePcClient::addUnivers);
    ...
    connect(btnCancel, &QPushButton::clicked, this, &InterfacePcClient::showList);
    QPushButton* btnSave = new QPushButton("Enregistrer");
    btnSave->setObjectName("btnGreen");
    connect(btnSave, &QPushButton::clicked, this, &InterfacePcClient::saveEquipment);
    ...
    connect(scenesUniversCombo, &QComboBox::currentIndexChanged, this, &InterfacePcClient::onScenesUniversChanged);
    btnResetSliders = new QPushButton("Reset 0");
    btnResetSliders->setObjectName("btnGrey");
    connect(btnResetSliders, &QPushButton::clicked, this, &InterfacePcClient::resetSliders);
    ...
    connect(scenesCombo, &QComboBox::currentIndexChanged, this, &InterfacePcClient::onSceneSelectionChanged);
    btnRenameScene = new QPushButton("Renommer");
    btnRenameScene->setObjectName("btnGrey");
    btnRenameScene->setEnabled(false);
    connect(btnRenameScene, &QPushButton::clicked, this, &InterfacePcClient::onRenameSceneClicked);
    btnDeleteScene = new QPushButton("Supprimer");
    btnDeleteScene->setObjectName("btnRed");
    btnDeleteScene->setEnabled(false);
    connect(btnDeleteScene, &QPushButton::clicked, this, &InterfacePcClient::onDeleteSceneClicked);
    QPushButton* btnSaveScene = new QPushButton("Enregistrer la Scène");
    btnSaveScene->setObjectName("btnGreen");
    connect(btnSaveScene, &QPushButton::clicked, this, &InterfacePcClient::saveCurrentScene);
    ...
    connect(btnBackToEquipments, &QPushButton::clicked, this, &InterfacePcClient::showList);
    ...
    connect(btnConnectTCP, &QPushButton::clicked, this, &InterfacePcClient::on_btnConnect_clicked);
    ...
    liveScenesList->setStyleSheet(
        "QListWidget::item { padding: 15px; font-size: 14px; border-bottom: 1px solid #444; } "
        "QListWidget::item:selected { background-color: #107c7c; color: white; font-weight: bold; }"
    );
    connect(liveScenesList, &QListWidget::itemSelectionChanged, this, &InterfacePcClient::onLiveSceneSelected);
    ...
    connect(btnLaunchLiveScene, &QPushButton::clicked, this, &InterfacePcClient::onLaunchButtonClicked);
...
void InterfacePcClient::setupStyle()
{
    setStyleSheet(R"(
        ...
        QPushButton#btnSeaGreen  { background-color: #20b2aa; color: white; border: none; }
        QPushButton#btnSeaGreen:hover  { background-color: #149999; }

        QPushButton#btnDarkCyan  { background-color: #008b8b; color: white; border: none; }
        QPushButton#btnDarkCyan:hover  { background-color: #149999; }

        QPushButton#btnDeepTeal  { background-color: #005f5f; color: white; border: none; }
        QPushButton#btnDeepTeal:hover  { background-color: #149999; }
...
void InterfacePcClient::onUniversSelectionChanged()
{
...
void InterfacePcClient::addUnivers()
{
    QDialog dialog(this);
    ...
    for (const auto& u : universList)
        if (u.numero >= prochainNumero) prochainNumero = u.numero + 1;

    QLineEdit* numEdit = new QLineEdit(&dialog);
    numEdit->setText(QString::number(prochainNumero));
    QLineEdit* ipEdit = new QLineEdit(&dialog);
    QRegularExpression rx("^192\\.168\\.1\\.([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-4])$");
    ipEdit->setValidator(new QRegularExpressionValidator(rx, this));
...
void InterfacePcClient::editUnivers()
{
    ...
    QLineEdit* numEdit = new QLineEdit(QString::number(u.numero), &dialog);
    QLineEdit* ipEdit  = new QLineEdit(u.ip, &dialog);
    QRegularExpression rx("^192\\.168\\.1\\.([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-4])$");
    ipEdit->setValidator(new QRegularExpressionValidator(rx, this));
    ipEdit->setPlaceholderText("192.168.1.X");
...
void InterfacePcClient::deleteUnivers()
{
    int row = uiUniversList->currentRow();
    if (row < 0 || row >= universList.size()) return;
...
void InterfacePcClient::showAddForm()
{
...
void InterfacePcClient::showList()
{
    stackedWidget->setCurrentWidget(listPage);
}

void InterfacePcClient::showScenesPage()
{
...
void InterfacePcClient::showLivePage()
{
...
void InterfacePcClient::saveEquipment()
{
    EquipmentData eq;
...
void InterfacePcClient::onScenesUniversChanged()
{
...
void InterfacePcClient::saveCurrentScene()
{
...
void InterfacePcClient::onSceneSelectionChanged()
{
...
void InterfacePcClient::resetSliders()
{
    for (int i = 0; i < 512; ++i)
        dmxSliders[i].slider->setValue(0);
}

void InterfacePcClient::onRenameSceneClicked()
{
    int idScene = scenesCombo->currentData().toInt();
    if (idScene == -1) {
    ...
    QString nouveauNom = QInputDialog::getText(this, "Renommer la scène",
                             "Nouveau nom :", QLineEdit::Normal, scenesCombo->currentText(), &ok);
...
void InterfacePcClient::onDeleteSceneClicked()
{
    int idScene = scenesCombo->currentData().toInt();
    if (idScene == -1) {
        statusLabel->setText("Aucune scène sélectionnée. Veuillez sélectionner une scène déjà existante !");
...
void InterfacePcClient::onLiveSceneSelected()
{
...
void InterfacePcClient::onLaunchButtonClicked()
{
...
void InterfacePcClient::on_btnConnect_clicked()
{
