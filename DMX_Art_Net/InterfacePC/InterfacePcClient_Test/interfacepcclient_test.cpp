...
void MainWindow::addUnivers() {
    ...
    if (dialog.exec() == QDialog::Accepted && !numEdit->text().isEmpty()) {
        if (bdd.enregistrerUnivers(numEdit->text().toInt(), ipEdit->text()))
            ...
            QMessageBox::warning(this, "Erreur", "Impossible d'enregistrer l'univers.");
    }
}
    ...
    if (row >= 0 && row < universList.size()) {
        ...
        if (dialog.exec() == QDialog::Accepted && !numEdit->text().isEmpty()) {
            if (bdd.modifierUnivers(u.idUnivers, numEdit->text().toInt(), ipEdit->text()))
                ...
                QMessageBox::warning(this, "Erreur", "Impossible de modifier cet univers.");
        }
    }
}
    ...
    if (row >= 0 && row < universList.size()) {
        if (QMessageBox::question(this, "Confirmation", "Supprimer cet univers ?") == QMessageBox::Yes) {
            if (bdd.supprimerUnivers(universList[row].idUnivers))
                ...
                QMessageBox::warning(this, "Erreur", "Impossible de supprimer cet univers.");
        }
    }
}
...
void MainWindow::showAddForm() {
    if (universList.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Veuillez créer au moins un Univers DMX.");
        return;
    }
...
void MainWindow::saveCurrentScene() {
    ...
    if (valeursAEnregistrer.isEmpty()) {
        QMessageBox::information(this, "Info", "Aucune valeur > 0 à enregistrer.");
        return;
    }

    if (bdd.enregistrerScene(sceneName, valeursAEnregistrer)) {
        QMessageBox::information(this, "Succès", "Scène '" + sceneName + "' sauvegardée !");
        refreshScenesList();
    } else {
        QMessageBox::critical(this, "Erreur", "Impossible d'enregistrer la scène.");
    }
}
...
void MainWindow::lancerScene(int idScene) {
    ...
    //     QMessageBox::warning(this, "Erreur", "Connectez-vous au serveur TCP.");
    //     return;
    // }
    ...
    //     int indexCanal = it.key() - 1;
    //     if(indexCanal >= 0 && indexCanal < 512) dmxFrame[indexCanal] = (static_cast<char>(it.value()));
    // }
    quint16 taille = 0;
    // quint16 universAEnvoyer = static_cast<quint16>(numeroUnivers);
    QBuffer tampon;
    char commande = 'P';
...
void MainWindow::on_btnConnect_clicked() {
    if (btnConnectTCP->text() == "Connexion") {
        ...
        if (ip.isEmpty() || port <= 0) {
            QMessageBox::warning(this, "Erreur", "IP/Port invalide.");
            return;
        }
        socketClient.connectToHost(ip, port);
        socketClient.waitForConnected(3000);
    } else {
...
void MainWindow::saveEquipment() {
    ...
    if (resultat) {
        ...
        QMessageBox::critical(this, "Erreur", "Échec de l'opération.");
    }
}
