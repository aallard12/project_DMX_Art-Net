    ...
    , ui(new Ui::InterfacePcClient)
{
    ...
    claude = new ClaudeClient(this);
    connect(claude, &ClaudeClient::analyseTerminee, this, &InterfacePcClient::onAnalyseClaudeTerminee);
    connect(claude, &ClaudeClient::erreur,          this, &InterfacePcClient::onErreurClaude);
 ...
 * @details Ouvre un sélecteur de fichier. Si c'est un JSON, le parse directement.
 *          Si c'est un PDF ou un fichier texte, envoie le contenu à Claude pour analyse..
 */
void InterfacePcClient::on_pushButtonImport_clicked()
{
    QString cheminFichier = QFileDialog::getOpenFileName(
                this, "Importer un équipement", "",
                "Fichiers supportés (*.json *.txt *.pdf);;JSON (*.json);;Texte (*.txt);;PDF (*.pdf)");

    if (!cheminFichier.isEmpty()) {

        if (cheminFichier.endsWith(".json", Qt::CaseInsensitive)) {
            // Import JSON direct — comportement existant
            QFile fichier(cheminFichier);
            if (fichier.open(QIODevice::ReadOnly)) {
                QJsonDocument doc = QJsonDocument::fromJson(fichier.readAll());
                fichier.close();
                if (!doc.isNull() && doc.isObject())
                    remplirFormulaireDepuisJson(doc.object());
                else {
            ...
            // PDF ou texte — envoi à Claude
            QFile fichier(cheminFichier);
            if (fichier.open(QIODevice::ReadOnly)) {
                QString contenu = QString::fromUtf8(fichier.readAll());
                fichier.close();
                statusLabel->setText("⏳ Analyse par Claude en cours...");
                statusLabel->setStyleSheet("color: white; font-weight: bold; font-size: 30px;");
                claude->analyserFicheTechnique(contenu);
            } else {
...
void InterfacePcClient::remplirFormulaireDepuisJson(const QJsonObject& doc)
{
    QJsonObject equipObj = doc.value("equipement").toObject();
    clearForm();
    ...
    for (const auto& canalVal : canaux) {
        ...
        for (const auto& funcVal : canalObj.value("fonctions").toArray()) {
...
void InterfacePcClient::onAnalyseClaudeTerminee(const QJsonObject& equipementJson)
{
    remplirFormulaireDepuisJson(equipementJson);
    statusLabel->setText("✅ Fiche technique analysée avec succès");
    statusLabel->setStyleSheet("color: green; font-weight: bold; font-size: 30px;");
    QTimer::singleShot(5000, this, [this]() { statusLabel->setText(""); });
}

void InterfacePcClient::onErreurClaude(const QString& message)
{
