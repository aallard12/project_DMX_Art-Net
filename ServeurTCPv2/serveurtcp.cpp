...
///
/// Cas blackout : la scène existe mais ne possède aucun canal dans PILOTE.
///   → Le lien SCENES↔UNIVERS passant exclusivement par PILOTE, il est
///     impossible de remonter à un univers précis sans entrée dans PILOTE.
///     Tous les univers sont donc mis à zéro (comportement sûr et cohérent).
///
/// Cas normal  : les canaux et leurs valeurs sont récupérés via la jointure
///   PILOTE → CANAUX → EQUIPEMENTS → UNIVERS, puis appliqués à la trame.
void ServeurTcp::envoyerScene(int idScene)
{
    // ─────────────────────────────────────────────────────────────────────
    // Étape 1 — S'assurer que la scène existe dans SCENES.
    // ─────────────────────────────────────────────────────────────────────
    ...
    {
        // ─────────────────────────────────────────────────────────────────
        // Étape 2 — Détecter si la scène est un blackout :
        //           aucune entrée dans PILOTE pour cet idScene.
        // ─────────────────────────────────────────────────────────────────
        ...
        else if (isBlackout)
        {
            // ─────────────────────────────────────────────────────────────
            // Étape 3a — BLACKOUT.
            //   Aucune entrée PILOTE → remontée vers UNIVERS impossible.
            //   On cible tous les univers connus et on envoie des trames
            //   de 512 octets à 0.
            // ─────────────────────────────────────────────────────────────
        ...
        {
            // ─────────────────────────────────────────────────────────────
            // Étape 3b — SCÈNE ACTIVE.
            //   Jointure complète pour récupérer, pour chaque canal :
            //   l'IP et le numéro de l'univers, l'adresse de départ de
            //   l'équipement, le numéro relatif du canal et sa valeur.
            // ─────────────────────────────────────────────────────────────
            QSqlQuery query;
            query.prepare(
            ...
            {
                QSet<QString> clearedKeys;  // Univers déjà remis à 0 pour cette scène.
                QSet<QString> modifiedKeys; // Univers ayant reçu au moins une valeur.

                while (query.next())
                {
                    const QString ip     = query.value("adresseIp").toString().trimmed();
                    const int     univ   = query.value("numeroUnivers").toInt();
                    const int     offset = query.value("adresseDepart").toInt(); // base 1
                    const int     canal  = query.value("numeroCanal").toInt();   // base 1
                    const int     val    = query.value("valeurCanaux").toInt();
                    const QString key    = ip + "_" + QString::number(univ);

                    // Premier passage sur cet univers → remise à zéro pour
                    // ne pas conserver les résidus d'une scène précédente.
                    ...
                    // Index DMX absolu en base 0 :
                    // (adresseDepart - 1) + (numeroCanal - 1)
                ...
                // ─────────────────────────────────────────────────────────
                // Étape 4 — Envoi des paquets Art-Net pour chaque univers
                //           modifié par la scène.
                // ─────────────────────────────────────────────────────────
